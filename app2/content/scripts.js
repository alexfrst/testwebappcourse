...
const queryObjects = async (name) =>{
    const doc = {name:name}
    return fetch(`https://testwebappcourse.herokuapp.com/app2/data2`,{body: JSON.stringify(doc) ,method: "POST", headers: {"Content-type": "application/json; charset=UTF-8"}}).then(function(result) {
        return result.json();
    }).then(function(json) {
        return(json.data)
    });
...
const sendObject = async (figure, c, intercept) =>{
    ...
    fetch('https://testwebappcourse.herokuapp.com/app2/data', {
        method: "POST",
